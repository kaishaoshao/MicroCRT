...
#if defined(__riscv_mul) || defined(__riscv_zmmul)
...
//
// Configuration of 16-bit floating type.  Detect support for
// _Float16 be asking whether __builtin_fabsf16 is defined, which
// works for at least the GNU compiler.
//
#if defined(__has_builtin)
  #if __has_builtin(__builtin_fabsf16)
    //
    // Configuration of 16-bit floating type name.
    //
...
// expands to 'unsigned long' for RISC-V.  This doesn't match the 32-bit
...
//
  ...
  #define __SEGGER_RTL_UMULL_X(x0, x1)                                                           \
    ({ unsigned __thi, __tlo;                                                                    \
      __SEGGER_RTL_UMULL(__tlo, __thi, x0, x1);                                                  \
      (__SEGGER_RTL_U64)(((__SEGGER_RTL_U64)__thi << 32) + __tlo);                               \
    })
...
//
// Kill information about value held in X.
//
#define __SEGGER_RTL_KILL(X) __asm__("# %0" : "+r"(X))

//
// Configuration of division-remainder.
//
#if defined(__GNUC__) && !__SEGGER_RTL_CORE_HAS_FUSED_DIVREM
  //
  #define __SEGGER_RTL_DIVMOD_U32(Q, R, N, D)                                   \
    do {                                                                        \
      Q = (N) / (D);                                                            \
      __SEGGER_RTL_KILL(Q); /*Compiler left clueless as to Q */                 \
      R = (N) - (Q)*(D);                                                        \
    } while (0)
...
static __SEGGER_RTL_ALWAYS_INLINE unsigned __SEGGER_RTL_DIFF_INDEX_func(__SEGGER_RTL_WORD x, __SEGGER_RTL_WORD y) {
  //
#if __SEGGER_RTL_CORE_HAS_ISA_SIMD && defined(__has_builtin) && __has_builtin(__builtin_riscv_bitrev)
  //
...
#if __SEGGER_RTL_COMPILING_LIBRARY
static __SEGGER_RTL_ALWAYS_INLINE __SEGGER_RTL_BOOL __SEGGER_RTL_atomic_is_lock_free(unsigned size, const volatile void *ptr) {
  ...
  case 2:  return ((unsigned long)ptr & 1) == 0;   // Misaligned half-words are not lock free
  case 4:  return ((unsigned long)ptr & 3) == 0;   // Misaligned words are not lock free
  case 8:  return ((unsigned long)ptr & 7) == 0;   // Misaligned words are not lock free
