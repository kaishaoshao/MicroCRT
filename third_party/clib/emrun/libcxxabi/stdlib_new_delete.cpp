...
#define _LIBCPP_WEAK __attribute__((weak))
#endif
...
static void* operator_new_aligned_impl(std::size_t size, std::align_val_t alignment) {
  ...
  void* p;
  while ((p = /*std::__libcpp_*/aligned_alloc(static_cast<std::size_t>(alignment), size)) == nullptr) {
    std::new_handler nh = std::get_new_handler();
    if (nh)
      nh();
    else
...
_LIBCPP_WEAK void operator delete(void* ptr, std::align_val_t) noexcept { /*std::__libcpp_aligned_*/free(ptr); }
