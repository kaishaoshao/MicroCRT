...
### 1.5 profile 和 capability 必须同时支持

后续实现必须同时支持两种构建方式：

1. profile-first
   例如：
   `default / integer / full`
2. capability-first
   例如：
   直接关闭 `PRINTF_CAP_LONG_LONG`
   直接关闭 `PRINTF_CAP_WCHAR`
...
那么显式 capability 必须覆盖 profile 默认值。

### 1.6 如果兼容 Arm 风格裁剪，hook 只能放在稳定边界

允许后续兼容 Arm 那种“替换 formatter 行为”的裁剪思想，但不应把 hook 打散到 parser/core 主循环中。

允许的 hook/override 边界：

* conversion dispatcher 后的 family 入口
* integer/float/text backend helper
* output sink / adapter

不允许的 hook/override 边界：

* `printf_core.inc` 主循环内部
* parser 状态机内部
* 到处分布的弱符号碎片
...
default helper
or
ops/strategy override
```

而不是重新回到“很多历史钩子函数散落各处”的结构。
...
当前 `stream` adapter 和 `FILE` 的关系要特别说明：

```text
现在
  FILE          还是保守 stdio 结构
  __printf_out  已经开始具备更强 backend 能力

所以
  printf route 可以先独立升级 backend
  而不必立刻改变 FILE ABI
```
...
FILE
  ...
  -> flush
  -> finalize
  -> optional backend strategy state
...
* `printf_out` 先比 `FILE` 更强
* FILE-backed `write` 先通过 `put` fallback

但长期目标应是：

* `FILE` 和 `printf_out` 对齐到同一套 backend 语义
...
printf / fprintf / snprintf / ...
  -> __printf_vformat_stream / __printf_vformat_cstr*
  -> __printf_core_default / integer / full
  -> printf_core.inc
     -> __printf_scan_literal
     -> __printf_parse_*
     -> __printf_dispatch_conversion
...
当前 `printf_core_impl.inc` 的装配顺序应理解为：

```text
1. shared primitive
   ultoa_invert.c

2. parser/text family
   _printf_positional.c
   _printf_text_core.c
   _printf_parser.c
   _printf_char_core.c
...
6. common core template
   printf_core.inc
```

这不是“按文件名罗列”，而是在表达依赖方向：

* backend 先于 entry
* family 先于 final dispatcher
* dispatcher 先于 core loop
